...
**Model Discovery**
The runtime step that resolves a Routing Ladder tier to a concrete model id at invocation time, by querying the active backend (`claude` / `opencode`) CLI for the models it actually offers. Tier→model resolution is operator-config-driven: the operator supplies a tier→model mapping, and discovery validates that mapping against the backend's real catalog (rejecting mappings to unavailable models). bluei ships **no default tier→model mapping** — vendor model ids rot on the vendor's schedule, so discovery (with an operator-supplied, validated mapping) is the only non-rotting source of truth. Distinct from the Model Governor (which emits the tier ordinal) and from the Routing Ladder (the ordinal set discovery resolves over). Proven via mocked-subprocess tests; the Benchmark pins a `MockModelDiscovery` for reproducibility.
_Avoid_: model resolver (too generic), model registry (implies a static shipped table — the rotting kind this replaces), model picker.
