#!/usr/bin/env python3
"""Tests for bluei engine runner path hygiene."""
...
def test_runner_defaults_no_longer_point_to_pr_automation():
    assert "pr-automation" not in str(DEFAULT_STATE)
...
def test_generated_cycle_commands_use_current_runner_path():
    args = _args()
    issue_cmd = build_issue_cycle_command(args)
    reconcile_cmd = build_reconcile_only_command(args)
    verify_cmd = build_verification_only_command(args)

    runner_path = str(RUNNER_PATH)
    assert runner_path in issue_cmd
    assert runner_path in reconcile_cmd
    assert runner_path in verify_cmd
    assert "pr-automation/bluei/engine/cli.py" not in issue_cmd
    assert "pr-automation/bluei/engine/cli.py" not in reconcile_cmd
    assert "pr-automation/bluei/engine/cli.py" not in verify_cmd
...
def test_git_commit_all_reports_no_changes_without_failing(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(git_commit_all.__globals__, "run_capture", fake_run_capture)

    result = git_commit_all(repo, "demo commit", log_file, dry_run=False)
...
def test_verify_fix_closed_matches_specific_finding_not_whole_file(
    ...
    monkeypatch.setitem(
        verify_fix_closed.__globals__,
        "discover_findings",
        lambda *args, **kwargs: [other_same_file],
    )

    assert verify_fix_closed(worktree, target, log_file) is True
