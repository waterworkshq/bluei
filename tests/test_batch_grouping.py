...
class TestIsIsolated:
    def test_migrations_excluded(self, make_finding):
    ...
    def test_normal_file_not_isolated(self, make_finding):
        f = make_finding(path="zerver/lib/message.py")
        config = {"file_patterns": ["**/migrations/*.py"]}
        assert is_isolated(f, config) is False

    def test_middleware_excluded(self, make_finding):
        f = make_finding(path="zerver/middleware/logging.py")
        config = {"file_patterns": ["**/middleware*.py"]}
        assert is_isolated(f, config) is True

    def test_empty_isolation_config(self, make_finding):
        f = make_finding()
        assert is_isolated(f, {}) is False

    def test_no_file_patterns(self, make_finding):
        f = make_finding()
...
class TestChunkFindings:
    def test_single_chunk_under_limit(self, make_finding):
    ...
    def test_splits_at_max_batch_size(self, make_finding):
    ...
    def test_splits_at_max_files(self, make_finding):
        findings = [
            make_finding(finding_id=f"f{i:03d}", path=f"zerver/file_{i}.py")
            for i in range(8)
        ]
        rule = BatchRule(
            rule_pattern="ruff-c408", max_batch_size=20, max_files_per_batch=5
        )
        chunks = chunk_findings(findings, rule)
    ...
    def test_single_finding(self, make_finding):
    ...
    def test_high_density_many_findings_few_files_one_batch(self, make_finding):
        """20 findings in 5 files → high density → large batch, all in one chunk."""
        paths = [f"zerver/file_{i % 5}.py" for i in range(20)]
        findings = [
            make_finding(finding_id=f"f{i:03d}", path=paths[i]) for i in range(20)
        ]
        rule = BatchRule(
            rule_pattern="ruff-c408", max_batch_size=20, max_files_per_batch=15
        )
    ...
    def test_low_density_many_findings_many_files_smaller_batches(self, make_finding):
    ...
    def test_all_findings_one_file_high_auto_capped(self, make_finding):
    ...
    def test_each_finding_unique_file_auto_equals_max_files(self, make_finding):
        ...
        rule = BatchRule(
            rule_pattern="ruff-c408", max_batch_size=30, max_files_per_batch=15
        )
...
class TestCheckBatchConflicts:
    def test_no_conflicts_different_files(self, make_finding):
    ...
    def test_conflict_same_file_nearby_lines(self, make_finding):
        f1 = make_finding(finding_id="f1", path="a.py", line=10)
    ...
    def test_no_conflict_same_file_far_lines(self, make_finding):
        f1 = make_finding(finding_id="f1", path="a.py", line=10)
    ...
    def test_multiple_conflicts(self, make_finding):
        f1 = make_finding(finding_id="f1", path="a.py", line=10)
...
class TestGroupFindingsForBatch:
    def test_groups_same_rule_into_one_batch(self):
        ...
        candidates = [
            make_candidate(
                fid=f"f{i:03d}", rule="ruff-c408", path=f"zerver/file_{i}.py"
            )
            for i in range(10)
        ]
        rules = [
    ...
    def test_isolation_excludes_migrations(self):
        ...
        candidates = [
            make_candidate(
                fid="f001", rule="ruff-c408", path="zerver/migrations/0001_initial.py"
            ),
    ...
    def test_wildcard_rule_catches_remaining(self):
        ...
        rules = [
            BatchRule(
                rule_pattern="ruff-c408", enabled=True, group_by="rule", priority=1
            ),
            BatchRule(
                rule_pattern="ruff-*",
                enabled=True,
...
class TestBatchGroupModel:
    def test_from_solo(self, make_finding):
        f = make_finding()
        issue = make_issue()
    ...
    def test_from_findings(self, make_finding):
        findings = [make_finding(finding_id=f"f{i:03d}") for i in range(3)]
        issues_map = {
            f.finding_id: make_issue(finding_id=f.finding_id) for f in findings
        }
        rule = BatchRule(
            rule_pattern="ruff-c408",
            group_by="rule",
    ...
    def test_file_count(self, make_finding):
        findings = [
    ...
    def test_pr_title_solo(self, make_finding):
        f = make_finding(path="zerver/lib/message.py")
        issue = make_issue()
        bg = BatchGroup.from_solo(issue, f)
        title = bg.pr_title()
    ...
    def test_pr_title_batch(self, make_finding):
        findings = [make_finding(finding_id=f"f{i:03d}") for i in range(5)]
        issues_map = {
            f.finding_id: make_issue(finding_id=f.finding_id) for f in findings
        }
    ...
    def test_pr_body_solo(self, make_finding):
        f = make_finding(path="zerver/lib/message.py")
        issue = make_issue()
        bg = BatchGroup.from_solo(issue, f)
        body = bg.pr_body()
    ...
    def test_pr_body_batch(self, make_finding):
        findings = [
            make_finding(finding_id=f"f{i:03d}", path=f"file_{i}.py") for i in range(3)
        ]
        issues_map = {
            f.finding_id: make_issue(finding_id=f.finding_id) for f in findings
        }
    ...
    def test_to_record(self, make_finding):
        f = make_finding()
        issue = make_issue()
        bg = BatchGroup.from_solo(issue, f)
        record = bg.to_record()
...
class TestBatchStatePersistence:
    ...
    def test_update_record(self, tmp_path):
        ...
        updated = update_batch_record(
            path, "batch-001", {"status": "pr_created", "pr_number": 42}
        )
