...
def test_full_directive_seeding_e2e_flow(make_finding):
    """
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert rec["fix_attempts"] == 1, (
            f"Expected fix_attempts=1, got {rec['fix_attempts']}"
        )
        ...
        assert len(allowed) == 1, (
            f"Expected 1 allowed, got {len(allowed)} (suppressed={suppressed})"
        )
        assert len(suppressed) == 0
        print()
        print(
            "Step 5 ✅ – filter_findings_by_cooldown: NOT suppressed (base cooldown elapsed)"
        )
        ...
        print()
        print(
            "Step 6 ✅ – filter_findings_by_cooldown: IS suppressed (failure_count=2, 16h effective)"
        )
        ...
        print()
        print(
            "Step 7 ✅ – filter_findings_by_cooldown: NOT suppressed after failure_count reset"
        )
        ...
        print()
        print(
            "Step 9 ✅ – render_claude_fix_prompt includes ## Prior context + ## Fix history"
        )
        ...
        print()
        print(
            "Step 10 ✅ – fix_success=True + failure_count=0 resets effective cooldown to base"
        )
