...
class TestCrossRuleBatching:
    ...
    def test_cross_rule_pr_title(self, make_finding):
        ...
        assert "3 linter specks" in title
        assert "3 rules" in title

    def test_cross_rule_pr_body_grouped_by_rule(self, make_finding):
        ...
        assert "2 specks" in body  # ruff-c408 has 2
    ...
    def test_cross_rule_respects_isolation(self):
        ...
        candidates = [
            make_candidate(
                fid="f001", rule="ruff-c408", path="zerver/lib/a.py", line=1
            ),
            make_candidate(
                fid="f002", rule="ruff-b904", path="zerver/lib/b.py", line=2
            ),
            make_candidate(
                fid="f003", rule="ruff-c408", path="zerver/migrations/0001.py", line=3
            ),
        ]
        batches = group_findings_for_batch(candidates, [cross_rule])
        ...
        migration_solos = [
            b for b in solo if b.findings[0].path == "zerver/migrations/0001.py"
        ]
    ...
    def test_cross_rule_disabled_by_default(self):
        ...
        c408_batches = [
            b for b in batches if any(f.rule == "ruff-c408" for f in b.findings)
        ]
        ...
        b904_batches = [
            b for b in batches if any(f.rule == "ruff-b904" for f in b.findings)
        ]
        assert all(b.is_solo for b in b904_batches)

    def test_cross_rule_title_single_rule_fallback(self, make_finding):
...
class TestSeverityBatching:
    ...
    def test_severity_high_max_5(self):
        ...
        candidates = [
            make_candidate(fid=f"f{i:03d}", rule="ruff-b904", path=f"f{i}.py", line=i)
            for i in range(12)
        ]
        batches = group_findings_for_batch(candidates, [rule])
        multi = [b for b in batches if not b.is_solo]
    ...
    def test_severity_low_max_30(self):
        ...
        candidates = [
            make_candidate(fid=f"f{i:03d}", rule="ruff-b007", path=f"f{i}.py", line=i)
            for i in range(25)
        ]
        batches = group_findings_for_batch(candidates, [rule])
        multi = [b for b in batches if not b.is_solo]
    ...
    def test_severity_ordering(self):
        ...
        candidates = [
            make_candidate(
                fid="f001", rule="ruff-s311", path="a.py", line=1
            ),  # critical
            make_candidate(
                fid="f002", rule="ruff-s311", path="b.py", line=2
            ),  # critical
        ...
        s311_batches = [
            b for b in batches if any(f.rule == "ruff-s311" for f in b.findings)
        ]
        c408_batches = [
            b for b in batches if any(f.rule == "ruff-c408" for f in b.findings)
        ]
        b007_batches = [
            b for b in batches if any(f.rule == "ruff-b007" for f in b.findings)
        ]
