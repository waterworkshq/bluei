...
# Ensure bluei is importable
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from bluei.app.healer import (
...
class TestHealerGitignoreHealing:
    ...
    def test_heal_gitignore_dry_run_does_not_write(self, git_repo):
        ...
        assert (
            not (git_repo / ".gitignore").exists()
            or "QA Agent" not in (git_repo / ".gitignore").read_text()
        )
...
class TestHealerArtifactRemoval:
    ...
    def test_heal_full_dry_run_reports_intent(self, git_repo):
        ...
        assert (
            len(result["gitignore_entries_added"]) > 0
            or len(result["artifacts_removed"]) > 0
        )
...
class TestHealerTransientDirtyPathsTopLevel:
    """Top-level transient directories detected in dirty paths."""

    def test_get_transient_dirty_paths_matches_top_level_component(self, git_repo):
        (git_repo / "coverage").mkdir()
        (git_repo / "coverage" / "report.txt").write_text("cov")
        healer = TransientArtifactHealer(git_repo)
        paths = healer.get_transient_dirty_paths()
        assert any("coverage" in p for p in paths)


class TestHealerExclusivelyTransientEmpty:
    """is_exclusively_transient_dirty with no dirty paths."""

    def test_empty_dirty_returns_false(self, git_repo):
        healer = TransientArtifactHealer(git_repo)
        assert healer.is_exclusively_transient_dirty() is False


class TestHealerRemoveArtifactsEdgeCases:
    """Artifact removal edge cases."""

    def test_heal_remove_artifacts_no_transient(self, git_repo):
        healer = TransientArtifactHealer(git_repo)
        changed, removed = healer.heal_remove_artifacts(dry_run=False)
        assert changed is False
        assert removed == []

    def test_heal_remove_artifacts_file_unlink(self, git_repo):
        (git_repo / ".coverage").write_text("cov data")
        healer = TransientArtifactHealer(git_repo)
        changed, removed = healer.heal_remove_artifacts(dry_run=False)
...
class TestHealerTransientDirtyNestedParts:
    """Nested paths under transient directories are detected."""

    def test_transient_dirty_paths_matches_nested_under_transient_dir(
        self, git_repo, git_commit_all
    ):
        (git_repo / "coverage").mkdir()
        (git_repo / "coverage" / ".gitkeep").write_text("")
        git_commit_all(git_repo, "track coverage dir")
        (git_repo / "coverage" / "myreports").mkdir()
        (git_repo / "coverage" / "myreports" / "index.html").write_text("report")
        healer = TransientArtifactHealer(git_repo)
        paths = healer.get_transient_dirty_paths()
        assert any("coverage" in p for p in paths)
