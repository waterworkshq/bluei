...
def test_discover_typescript_type_findings_detects_untyped_import(
    ...
    monkeypatch.setattr(
        linters, "run_capture", lambda *args, **kwargs: (2, compiler_output)
    )
...
def test_reconcile_issue_pr_link_reopens_issue_when_linked_pr_is_closed(
    monkeypatch, tmp_path
):
    issue = {
        "issue_id": "QA-0025",
    ...
    monkeypatch.setattr(
        "bluei.engine.commands.helpers.find_existing_github_pr",
        lambda *args, **kwargs: {
            "number": 33,
            "url": "https://github.com/example/repo/pull/33",
            "state": "CLOSED",
            "headRefName": "qa/live-abc123",
        },
    )
...
def test_choose_validation_baseline_prefers_worktree_when_repo_and_worktree_drift(
    tmp_path,
):
    repo_baseline = {
