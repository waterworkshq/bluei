...
BLUEI_CLI = ROOT / "bin" / "bluei"


def _run(*args, workdir: Path | None = None, timeout: int = 120) -> subprocess.CompletedProcess:
    """Run the bluei CLI directly and return the result."""
    cmd = [sys.executable, str(ROOT / "bin" / "bluei.py")] + list(args)
...
""")
    ...
    subprocess.run(["git", "config", "user.email", "test@bluei.dev"], cwd=path, capture_output=True)
    subprocess.run(["git", "config", "user.name", "Bluei Test"], cwd=path, capture_output=True)
...
class TestOnboardingE2E:
    ...
    def setup_method(self):
        self.test_dir = Path("/tmp/bluei-e2e-test")
        ...
        state_dir = ROOT / "repos" / "bluei-e2e-test"
        if state_dir.exists():
    ...
    def _get_state_dir(self, repo_name: str = "bluei-e2e-test") -> Path:
    ...
    def test_01_onboard_detects_language(self):
        ...
        config_path = ROOT / "repos" / "bluei-e2e-test" / "config.yaml"
    ...
    def test_02_scan_discovers_findings(self):
        ...
        result = _run(
            "run", "--repo", "bluei-e2e-test",
            "--phase", "issue-cycle", "--dry-run",
        )
        assert result.returncode == 0, f"issue-cycle failed: {result.stderr}"

        # Check output for findings/specks
        assert "findings" in result.stdout.lower() or "specks" in result.stdout.lower(), f"No findings/specks produced:\n{result.stdout}"
        assert "Findings:" in result.stdout or "Specks:" in result.stdout, f"Missing Findings/Specks count:\n{result.stdout}"
    ...
    def test_04_health_score_written(self):
        """Status.json or health history should contain health/vitality data."""
        _run(
            "onboard", "--repo", str(self.repo_path),
            "--name", "bluei-e2e-test",
            "--mode", "observe",
        ...
        # Health data may be in status.json current_counts or health_history.jsonl
        ...
        # health_score may or may not be in current_counts depending on the path taken
        health = counts.get("health_score")

        # If not in current_counts, check health_history.jsonl
        if health is None:
            health_history = self._get_state_dir() / "health_history.jsonl"
            if health_history.exists():
                lines = [l for l in health_history.read_text().splitlines() if l.strip()]
                if lines:
                    last = json.loads(lines[-1])
                    health = last.get("score")

        # Health should be available from at least one source
        assert health is not None, f"health/vitality not found in status.json or health_history"
