...
from bluei.engine.reforge import classify_finding, RefactorClass
...
def _load_findings():
    if not ZULIP_FINDINGS.exists():
        pytest.skip("zulip findings.jsonl not available — live data required")
    findings = []
    ...
    if not findings:
        pytest.skip("zulip findings.jsonl is empty")
    return findings
...
class TestZulipLiveMigration:
    ...
    def test_contextual_fix_count(self):
        ...
        findings = _load_findings()
        contextual = [
            f for f in findings if f.get("refactor_class") == "contextual_fix"
        ]
        assert len(contextual) > 500, (
            f"Expected 500+ contextual_fix, got {len(contextual)}"
        )
    ...
    def test_b904_is_contextual(self):
        ...
        assert len(b904) > 0
        contextual_b904 = [
            f for f in b904 if f.get("refactor_class") == "contextual_fix"
        ]
        assert len(contextual_b904) > len(b904) * 0.8, (
            f"Only {len(contextual_b904)}/{len(b904)} b904 are contextual_fix"
        )
    ...
    def test_b007_in_fixtures_is_simple(self):
        ...
        b007_fixtures = [
            f
            for f in findings
            if f.get("rule") == "ruff-b007" and "fixtures" in f.get("path", "")
        ]
    ...
    def test_c408_in_migrations_is_refactor(self):
        ...
        c408_migrations = [
            f
            for f in findings
            if f.get("rule") == "ruff-c408" and "migration" in f.get("path", "")
        ]
    ...
    def test_safe_to_autofix_matches_classification(self):
        ...
        assert len(unsafe_simple) == 0, (
            f"{len(unsafe_simple)} simple_fix findings still marked unsafe"
        )
