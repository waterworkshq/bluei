    ...
    def test_ranked_by_score_descending(self, mixed_env):
        ...
        make_repo(
            mixed_env["repo_path"],
            {
                ...
                "tsconfig.json": "{}",
                "package.json": json.dumps(
                    {"dependencies": {"express": "^4.0.0"}, "scripts": {}}
                ),
            },
        )
        ranked = mixed_env["engine"].detect_all_languages(mixed_env["repo_path"])
...
class TestDetectLanguageSecondaryLanguages:
    ...
    def test_typescript_primary_python_secondary(self, mixed_env):
        ...
        assert lang.name == "typescript", (
            f"Expected typescript primary, got {lang.name}"
        )
...
class TestInferBaselineChecksMixed:
    ...
    def test_python_plus_npm_infers_both(self, mixed_env):
        ...
        assert any("pytest" in c for c in cmd_strs), (
            f"Expected pytest in checks: {cmd_strs}"
        )
        assert any("vitest" in c or "npm test" in c for c in cmd_strs), (
            f"Expected vitest/npm test in checks: {cmd_strs}"
        )
    ...
    def test_python_plus_npm_deduplicated(self, mixed_env):
        ...
        checks = mixed_env["engine"].infer_baseline_checks(mixed_env["repo_path"], lang)
        test_cmds = [
            c for c in checks if any(k in c[-1] for k in ("pytest", "vitest", "test"))
        ]
    ...
    def test_triple_language_all_checks(self, mixed_env):
        ...
        assert any("vitest" in c or "npm test" in c for c in cmd_strs), (
            f"Expected npm/vitest in {cmd_strs}"
        )
    ...
    def test_python_primary_selects_python_template(self, mixed_env):
        ...
        make_repo(
            mixed_env["repo_path"],
            {
                ...
                "tsconfig.json": "{}",
                "package.json": json.dumps(
                    {"dependencies": {"react": "^18.0.0"}, "scripts": {}}
                ),
            },
        )
        lang = mixed_env["engine"].detect_language(mixed_env["repo_path"])
...
class TestBuildReviewItemsMixedLanguage:
    ...
    def test_mixed_language_review_item(self, mixed_env):
        ...
        from bluei.app.models import generate_id
        ...
        items = mixed_env["engine"].build_review_items(
            mixed_env["repo_path"], lang, config
        )
        mixed_item = next(
            (
                i
                for i in items
                if "mixed-language" in i.message.lower()
                or "secondary" in i.message.lower()
            ),
            None,
        )
        assert mixed_item is not None, f"Expected mixed-language review item in {items}"
        assert "python" in mixed_item.message
        assert "typescript" in mixed_item.message
    ...
    def test_single_language_no_mixed_review_item(self, mixed_env):
        ...
        mixed_item = next((i for i in items if "mixed-language" in i.message), None)
        assert mixed_item is None, (
            f"Unexpected mixed-language review item for single-language repo: {items}"
        )
...
class TestZulipStyleRepo:
    ...
    def test_zulip_style_detection(self, mixed_env):
        ...
        lang = mixed_env["engine"].detect_language(mixed_env["repo_path"])
        assert lang.name == "python", (
            f"Expected python primary (Zulip is Python-backend), got {lang.name}"
        )
        assert "typescript" in lang.secondary_languages
    ...
    def test_zulip_style_baseline_checks(self, mixed_env):
        ...
        cmd_strs = [" ".join(c) for c in checks]
        assert any("pytest" in c for c in cmd_strs), f"Expected pytest in {cmd_strs}"
        assert any("jest" in c or "npm test" in c for c in cmd_strs), (
            f"Expected jest/npm test in {cmd_strs}"
        )
