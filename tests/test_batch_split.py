...
class TestShouldSplitBatch:
    def test_no_failures_returns_false(self, make_finding):
    ...
    def test_49_percent_failure_returns_false(self, make_finding):
    ...
    def test_51_percent_failure_returns_true(self, make_finding):
        findings = [make_finding(finding_id=f"f{i}") for i in range(100)]
        fix_results = {}
        for i, f in enumerate(findings):
            fix_results[f.finding_id] = FixResult(
    ...
    def test_60_percent_failure_returns_true(self, make_finding):
    ...
    def test_max_depth_respected(self, make_finding):
        findings = [make_finding(finding_id=f"f{i}") for i in range(10)]
        fix_results = {
    ...
    def test_at_max_depth_minus_one_returns_true(self, make_finding):
        findings = [make_finding(finding_id=f"f{i}") for i in range(10)]
        fix_results = {
            f.finding_id: FixResult(finding_id=f.finding_id, status="failed")
            for f in findings
    ...
    def test_skipped_not_counted_as_failure(self, make_finding):
        findings = [make_finding(finding_id=f"f{i}") for i in range(10)]
        fix_results = {}
        for i, f in enumerate(findings):
            status = "skipped" if i < 5 else "success"
            fix_results[f.finding_id] = FixResult(
                finding_id=f.finding_id, status=status
            )
        batch = make_batch(findings, fix_results)
        assert should_split_batch(batch) is False

    def test_dict_fix_results_supported(self, make_finding):
        findings = [make_finding(finding_id=f"f{i}") for i in range(10)]
        fix_results = {}
        for i, f in enumerate(findings):
...
class TestSplitBatch:
    def test_split_single_failure_to_solo(self, make_finding):
    ...
    def test_split_two_failures_to_two_solos(self, make_finding):
    ...
    def test_split_six_failures_to_halves(self, make_finding):
    ...
    def test_split_respects_max_depth(self, make_finding):
        findings = [make_finding(finding_id=f"f{i}") for i in range(10)]
        fix_results = {
            f.finding_id: FixResult(finding_id=f.finding_id, status="failed")
            for f in findings
    ...
    def test_split_increments_retry_count(self, make_finding):
        findings = [make_finding(finding_id=f"f{i}") for i in range(5)]
        fix_results = {
            f.finding_id: FixResult(finding_id=f.finding_id, status="failed")
            for f in findings
    ...
    def test_split_batch_id_inherits_suffix(self, make_finding):
        findings = [make_finding(finding_id="f001")]
        fix_results = {"f001": FixResult(finding_id="f001", status="failed")}
        batch = make_batch(findings, fix_results)
    ...
    def test_split_all_success_returns_empty(self, make_finding):
        findings = [make_finding(finding_id=f"f{i}") for i in range(5)]
        fix_results = {
            f.finding_id: FixResult(finding_id=f.finding_id, status="success")
            for f in findings
...
class TestSplitOnConflicts:
    def test_no_conflicts_returns_original_batch(self, make_finding):
        findings = [
    ...
    def test_conflict_detection_and_split(self, make_finding):
    ...
    def test_nearby_findings_split_correctly(self, make_finding):
    ...
    def test_non_conflicting_findings_grouped_together(self, make_finding):
...
class TestHandleBatchFailure:
    def test_separates_successes_from_failures(self, make_finding):
        findings = [
    ...
    def test_split_history_recorded(self, make_finding):
        findings = [
            make_finding(finding_id="f001"),
            make_finding(finding_id="f002"),
        ]
    ...
    def test_no_failures_returns_empty_sub_batches(self, make_finding):
        findings = [
            make_finding(finding_id="f001"),
            make_finding(finding_id="f002"),
        ]
    ...
    def test_all_failures_no_commit(self, make_finding):
        findings = [
            make_finding(finding_id="f001"),
            make_finding(finding_id="f002"),
        ]
...
class TestCommitPartialBatch:
    def test_commits_successful_files(self, make_finding):
        findings = [
    ...
    def test_no_worktree_returns_false(self, make_finding):
        findings = [make_finding(finding_id="f001")]
        batch = make_batch(findings)
...
class TestRecoverInterruptedBatch:
    def test_recover_interrupted_batch_pushed(self, tmp_path):
        ...
        with patch(
            "bluei.engine.utils.run_capture", return_value=(0, "origin/qa/batch-001\n")
        ):
            batch = recover_interrupted_batch("batch-001", batches_file, worktree_root)
    ...
    def test_recover_interrupted_batch_not_pushed(self, tmp_path):
        ...
        with (
            patch("bluei.engine.utils.run_capture", return_value=(1, "")),
            patch("bluei.engine.utils.run_no_capture"),
        ):
            batch = recover_interrupted_batch("batch-002", batches_file, worktree_root)
