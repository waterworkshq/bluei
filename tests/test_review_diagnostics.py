...
def _cli_path() -> Path:
    return Path(__file__).parents[1] / "bin" / "bluei"


def _cli_module_path() -> Path:
    return Path(__file__).parents[1] / "bin" / "bluei.py"


def _cli_module():
    module_path = _cli_module_path()
    loader = SourceFileLoader("bluei_cli_module", str(module_path))
...
def test_cli_executable_exists():
    """Test that the bluei CLI is executable."""
    result = subprocess.run(
        [sys.executable, _cli_module_path(), "--help"], capture_output=True, text=True
    )
...
def test_doctor_command_runs():
    ...
    result = subprocess.run(
        [sys.executable, _cli_module_path(), "doctor"], capture_output=True, text=True
    )
    assert result.returncode in [0, 1]
    assert "bluei" in result.stdout.lower() or result.returncode == 1
...
def test_status_command_runs():
    ...
    cli_path = _cli_path()
    result = subprocess.run(
        [sys.executable, _cli_module_path(), "status"], capture_output=True, text=True
    )
    assert result.returncode == 0
    ...
    state.save_review_state(
        repo_name,
        {
            "prs": {
                "41": {
                    "attempts_used": 1,
                    "last_action_reason": "waiting for explicit approval",
                },
                "42": {
                    "attempts_used": 2,
                    "last_action_reason": "push rejected by remote",
                },
            }
        },
    )
...
def test_report_markdown_includes_pending_push_details(tmp_path):
    ...
    repo = Repo(
        config=RepoConfig(
            id="repo-1", name="demo", path="/tmp/demo", language="typescript"
        ),
            ...
            "pending_push_prs_detail": [
                {
                    ...
                    "branch": "qa/review-41",
                    "changed_files": ["a", "b"],
                    "push_target_branch": "qa/review-41",
                }
            ],
        },
    )
