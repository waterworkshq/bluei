#!/usr/bin/env python3
"""Review-specific state management.

ReviewStateManager is a self-contained state accessor for all review-
specific state surfaces (active PRs, review runs, findings, feedback,
learned rules, publish state, monitored safety, etc.).

Construction signature mirrors app.state.StateManager (repos_dir: Path)
so callers can swap one for the other without churn.

Dependency direction:
    review/ → common/   (dataclasses: ReviewRun, FeedbackEvent, ...)
    review/ → engine/   (atomic JSON I/O, jsonl, now_iso)
    review/ → review/   (self-contained; never reaches into app/)

Extracted from app/state.py during H5 review→app decoupling (2026-06-18).
"""
...
from bluei.common.models import ReviewRun, FeedbackEvent, now_iso
...
# ——— Versioned defaults for review state surfaces ———
# Moved verbatim from bluei/app/state.py during H5 decoupling.
...
class ReviewStateManager:
    """Manages review-specific persistent state.

    Self-contained: depends only on common/ (dataclasses) and engine/
    (atomic I/O, jsonl, now_iso). App.state.StateManager forwards
    review-specific calls here for backward compatibility.
    """
    ...
    # --- fix_patterns.jsonl --- (shared with app/runner.py; same path on disk)

    def get_fix_patterns_file(self, repo_name: str) -> Path:
        """Path to the fix-patterns JSONL store.

        Resolves to the same path as app.state.StateManager.get_fix_patterns_file.
        Kept on ReviewStateManager so review/feedback.py can resolve the path
        without depending on app/. The file is shared infrastructure — written
        by app/runner.py and read by review/feedback.py's confidence adjustment.
        """
