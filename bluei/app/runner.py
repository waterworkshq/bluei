#!/usr/bin/env python3
"""Run engine - wraps the bluei engine CLI."""
...
class RunEngine:
    """Executes QA runs by wrapping the bluei engine CLI."""
    ...
    def __init__(
        ...
        self.runner_path = config_manager.workspace / "bluei" / "engine" / "cli.py"
        ...
        if lock_dir.exists():
            ...
            for lock_file in lock_dir.glob("*.lock"):
                try:
                    ...
                    _logger.debug("Failed to remove stale lock file")
    ...
    def _resolve_backend(
        self, repo: Repo, requested_backend: Optional[str]
        ...
        if preferred == "deterministic":
            return {
                "logical_backend": "deterministic",
                "runner_engine": "deterministic",
                "template": None,
            }
        ...
        for backend in config.fallback_engines or [
            "claude",
            "opencode",
            "deterministic",
        ]:
    ...
    def _build_cli_args(
        ...
        repo: Repo,
        options: RunOptions,
        backend: Optional[Dict[str, Optional[str]]] = None,
    ) -> List[str]:
        """Build CLI arguments for the bluei engine CLI."""
        config = repo.config
        ...
        args = [
            ...
            config.path,
            "--state-file",
            str(self.state.get_state_file(repo_name)),
            "--log-file",
            str(self.config.workspace / "logs" / repo_name / "run.log"),
            "--findings-file",
            str(self.state.get_findings_file(repo_name)),
            "--issues-file",
            str(self.state.get_issues_file(repo_name)),
            "--worktree-root",
            str(self.config.repos_dir / repo_name / "worktrees"),
            "--status-file",
            str(self.state._get_state_dir(repo_name) / "status.json"),
            "--docs-index-file",
            str(self.state._get_state_dir(repo_name) / "docs_index.json"),
            "--run-phase",
            options.phase,
            # Limits
            "--open-issues-cap",
            str(config.limits.get("open_issues_cap", 20)),
            "--open-prs-cap",
            str(config.limits.get("open_prs_cap", 5)),
            "--max-prs-per-run",
            str(config.limits.get("max_prs_per_run", 2)),
            "--max-issues-per-run",
            str(config.limits.get("max_issues_per_run", 10)),
            "--max-files-changed",
            str(config.limits.get("max_files_changed", 5)),
            "--max-loc-diff",
            str(config.limits.get("max_loc_diff", 200)),
            "--max-fix-attempts-per-issue",
            str(config.limits.get("max_fix_attempts", 3)),
            # Cooldowns
            "--finding-cooldown-seconds",
            str(config.cooldowns.get("finding_seconds", 14400)),
            "--merge-cooldown-minutes",
            str(config.cooldowns.get("merge_minutes", 30)),
            "--staleness-threshold-seconds",
            str(config.cooldowns.get("staleness_seconds", 7200)),
            # Fix engine
            "--fix-engine",
            str(backend["runner_engine"]),
        ]
        ...
        if config.github.get("auto_rebase", {}).get("enabled", True):
            ...
            rebase_max = config.github.get("auto_rebase", {}).get(
                "max_prs_per_sweep", 5
            )
        ...
        # Pattern store path
        pattern_store_path = self.state.get_fix_patterns_file(repo_name)
        args.extend(["--pattern-store-path", str(pattern_store_path)])
        ...
        try:
            if tune_path.exists():
                ...
                if tuned:
                    ...
                    if overrides:
                        pass  # overrides applied in-place above; no separate flag needed
        except (OSError, json.JSONDecodeError, ValueError, IndexError):
            _logger.debug("Failed to read auto-tune config overrides")

        return args

    def _parse_output(self, output: str) -> Dict[str, int]:
        """Parse bluei engine output for metrics."""
        metrics = {
            "findings_detected": 0,
            "issues_created": 0,
            "fix_attempts": 0,
    ...
    def _run_review_cycle(
        ...
        if result.retry_failed_prs == 0 and result.findings_failed == 0:
            ...
            if tune_suggestion:
                reason = tune_suggestion.get(
                    "_reason", "retry/finding failure pattern detected"
                )
        ...
        signal_store = CycleSignalStore(
            self.config.workspace / "state" / "cycle_signals.json"
        )
        ...
        try:
            ...
            _trend_record = {
                ...
                "tune_cooldown_h": _tune_state.get("finding_cooldown_seconds", 0)
                // 3600
    ...
    def run(self, repo: Repo, options: RunOptions) -> RunResult:
        ...
        args = (
            self._build_cli_args(repo, options, resolved_backend)
        ...
        try:
            ...
            logging.debug("registry status update to RUNNING failed for %s", repo_name)
            pass

        try:
            if options.phase == "review-cycle":
                return self._run_review_cycle(
                    repo, run, log_dir, options.dry_run, options.allow_review_push
                )
            ...
            try:
                ...
                _tune_path = self.config.workspace / "state" / "auto_tune.json"
                if _tune_path.exists():
                    _tune_state = json.loads(_tune_path.read_text()).get(
                        "tuned_fields", {}
                    )
                _trend_record = {
                    "ts": now_iso(),
