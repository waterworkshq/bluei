...
# ── Standalone detection helpers (usable without OnboardEngine) ──────────
...
class OnboardEngine:
    ...
    def detect_all_languages(self, repo_path: Path) -> List[tuple[str, int]]:
        ...
        # Fallback: scan file extensions if no markers matched
    ...
    def detect_language(self, repo_path: Path) -> LanguageInfo:
        ...
        secondary = [
            lang for lang, _ in ranked[1:] if self._is_actionable_language(lang)
        ]
    ...
    def detect_git_remote(self, repo_path: Path) -> Dict[str, str]:
        ...
        Returns a dict with keys: 'url', 'host', 'owner', 'name', 'platform', 'detected'.
        Returns all-empty strings if no remote is found.
        """
        result: Dict[str, str] = {
            "url": "",
            "host": "",
        ...
        # Parse URL
        # Support both HTTPS (https://github.com/owner/repo.git) and SSH (git@github.com:owner/repo.git)
        match = re.match(
            r"(?:https?://|git@)([^:/]+)[:/]([^/]+)/([^/].*?)(?:\.git)?$", url
        )
        if match:
    ...
    def _infer_baseline_checks_for_language(
        ...
        commands: List[List[str]] = []
        pkg_mgr = package_manager or self.detect_package_manager(
    ...
    def infer_discovery_config(
        ...
        if use_docker:
            config["docker_hint"] = (
                "review container/service name before first live run"
            )
    ...
    def infer_safety_policy(
        ...
        mode = (
            options.mode
            if options.mode in {m.value for m in SafetyMode}
            else SafetyMode.OBSERVE.value
        )
        profile = (
            options.profile
            if options.profile in {p.value for p in SafetyProfile}
        ...
        if profile == SafetyProfile.CONSERVATIVE.value:
            notes.append(
                "Conservative profile keeps small diffs and low concurrency caps."
            )
    ...
    def detect_framework(self, repo_path: Path, language: str) -> Optional[str]:
        """Detect primary framework for a language.

        Delegates to the standalone detect_frameworks() and returns the first result.
        """
    ...
    def select_template(
        ...
        if language.name == "python":
            ...
            if (repo_path / "app.py").exists() or (repo_path / "main.py").exists():
                return "python-api"
            if (repo_path / "setup.py").exists() or (
                repo_path / "pyproject.toml"
            ).exists():
