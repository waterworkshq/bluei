...
class HealthEngine:
    ...
    def __init__(
        ...
        emergent_rules: Optional[List] = None,
    ):
        self.weights = weights or HealthWeights()
        self.emergent_rules: List = emergent_rules or []

    @classmethod
    def register_language_pack_mappings(
        cls,
        *,
        rule_to_component: Optional[Dict[str, str]] = None,
        category_inference: Optional[Dict[str, str]] = None,
    ) -> None:
        """Merge language-pack health mappings into the engine."""
        if rule_to_component:
            cls.RULE_TO_COMPONENT.update(
                {
                    str(rule).lower(): str(component)
                    for rule, component in rule_to_component.items()
                }
            )
        if category_inference:
            cls.CATEGORY_INFERENCE.update(
                {
                    str(keyword).lower(): str(component)
                    for keyword, component in category_inference.items()
                }
            )
    ...
    def _get_component(self, finding: Finding) -> str:
        ...
        # Check emergent rules first (emergent: prefix)
        if rule_lower.startswith("emergent:"):
            rule_id = (
                finding.rule.split(":", 1)[1] if ":" in finding.rule else finding.rule
            )
            for er in self.emergent_rules:
                if getattr(er, "rule_id", "") == rule_id and getattr(
                    er, "component_override", None
                ):
                    return er.component_override
            return "bug_quality"
    ...
    def calc_component_score(self, component: str, findings: List[Finding]) -> float:
        """Calculate score for a single component using logarithmic scaling."""
        component_findings = [
            f for f in findings if self._get_component(f) == component
        ]
        ...
        for severity, finding_list in by_severity.items():
            if finding_list:
                ...
                scaled_penalty = self._logarithmic_penalty(
                    base_penalty, len(finding_list)
                )
                total_penalty += scaled_penalty

        # Reduce penalty for emergent findings by their weight_multiplier
        for f in component_findings:
            if f.rule.startswith("emergent:"):
                rule_id = f.rule.split(":", 1)[1] if ":" in f.rule else f.rule
                for er in self.emergent_rules:
                    if getattr(er, "rule_id", "") == rule_id:
                        sev = self._get_severity(f)
                        base = penalties.get(sev, 2)
                        penalty_reduction = base * (
                            1.0 - getattr(er, "weight_multiplier", 0.5)
                        )
                        total_penalty -= penalty_reduction
                        break
    ...
    def calculate(
        ...
        code_quality_weight = (
            self.weights.bug_quality
            + self.weights.lint_quality
            + self.weights.technical_debt
    ...
    def get_findings_by_component(self, findings: List[Finding]) -> Dict[str, int]:
    ...
    def get_findings_by_category(self, findings: List[Finding]) -> Dict[str, int]:
        ...
        counts["code_quality"] = (
            counts.get("bug_quality", 0)
            + counts.get("lint_quality", 0)
            + counts.get("technical_debt", 0)
            + counts.get("maintainability", 0)
        )
        counts["test_coverage"] = counts.get("test_coverage", 0) + counts.get(
            "test_gaps", 0
        )
        return counts

    def get_findings_by_severity(self, findings: List[Finding]) -> Dict[str, int]:
    ...
    def prioritize_issues(
        self, findings: List[Finding], max_issues: int = 10
    ) -> List[PriorityIssue]:
    ...
    def save_health_snapshot(
        self, repo_name: str, health: HealthScore, findings_count: int, state_dir: Path
    ) -> None:
    ...
    def get_improvement_summary(
        ...
        for component in set(
            list(current_findings.keys()) + list(baseline_findings.keys())
        ):
