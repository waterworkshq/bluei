...
def _check_retry_pattern(
    records: List[Dict[str, Any]], threshold: int
) -> Optional[int]:
...
def compute_tune(
    ...
    """
    records = read_jsonl(stats_path, limit=20)
    ...
    if finding_hit is not None:
        current_cooldown = current.get("tuned_fields", {}).get(
        ...
        if new_cooldown > current_cooldown:
            ...
            override["_reason"] += (
                f"; findings_failed x{finding_hit} consecutive cycles"
            )
...
def adjust_replay_thresholds(
    ...
    records = read_jsonl(telemetry_path, limit=100)
    if not records:
        return {}
    ...
    for rule, rule_records in by_rule.items():
        ...
        for rec in reversed(recent):
            if (
                ...
                and rec.get("final_stage", "") != "cascade_exhausted"
            ):
...
def reset_tune(tune_path: Path) -> None:
    ...
    _save_tune_state(
        tune_path, {"tuned_fields": {}, "last_tune_ts": None, "reset_at": None}
    )
...
def flag_tune_success(tune_path: Path) -> None:
    ...
    if (
        not tuned
        or tuned == {"_recovery_step": "increased"}
        or tuned == {"_recovery_step": "halved_cooldown"}
    ):
        state["tuned_fields"] = {}
