...
class ReportGenerator:
    ...
    def __init__(
        self,
        pdf_report_skill_path: Optional[Path] = None,
        workspace: Optional[Path] = None,
    ):
        from .config import WORKSPACE as _DEFAULT_WS

        self.workspace = (workspace or _DEFAULT_WS).resolve()
        try:
            from .config import load_global_config

            cfg_skill = (
                load_global_config(self.workspace)
                .get("report", {})
                .get("pdf_skill_path")
            )
        except Exception:
        ...
        skill_path = cfg_skill or env_skill
        default_skill = self.workspace.parent / "skills" / "pdf-report"
        self.pdf_report_skill = (
            Path(skill_path).expanduser().resolve()
            if skill_path
            else (pdf_report_skill_path or default_skill).resolve()
        )
    ...
    def generate_markdown_report(
        ...
        if health:
            lines.extend(
                [
                    f"**Vitality:** {health.score}/100 ({self._format_score_band(health.score)})",
                    "",
                ]
            )
        else:
        ...
        if findings_by_category:
            ...
            for category, count in sorted(
                findings_by_category.items(), key=lambda x: -x[1]
            ):
        ...
        if review_care and review_care.get("active_managed_prs", 0) > 0:
            ...
            if any(
                ...
                if review_care.get("safety_cooldown_until"):
                    lines.append(
                        f"- **Safety Cooldown Until:** {review_care.get('safety_cooldown_until')[:19]}"
                    )
                lines.append(
                    f"- **Auto Rollback Active:** {review_care.get('auto_rollback_active', False)}"
                )
                if review_care.get("auto_rollback_reason"):
                    lines.append(
                        f"- **Auto Rollback Reason:** {review_care.get('auto_rollback_reason')}"
                    )
                if review_care.get("auto_rollback_triggered_at"):
                    lines.append(
                        f"- **Auto Rollback Triggered At:** {review_care.get('auto_rollback_triggered_at')[:19]}"
                    )
                if review_care.get("operator_action_required"):
                    lines.append(
                        f"- **Operator Action Required:** {review_care.get('operator_action_required')}"
                    )
                if review_care.get("operator_action_summary"):
                    lines.append(
                        f"- **Operator Action Summary:** {review_care.get('operator_action_summary')}"
                    )
            ...
            if review_care.get("pending_push_prs_detail"):
                ...
                for pr in review_care.get("pending_push_prs_detail", []):
                    lines.append(
                        f"| #{pr.get('pr_number', '?')} | {pr.get('branch', '?')} | {len(pr.get('changed_files', []))} | {pr.get('push_target_branch') or pr.get('branch', '?')} |"
                    )
                lines.append("")
            if review_care.get("failed_push_prs_detail"):
                ...
                for pr in review_care.get("failed_push_prs_detail", []):
                    lines.append(
                        f"| #{pr.get('pr_number', '?')} | {pr.get('branch', '?')} | {pr.get('push_status') or pr.get('status', '?')} |"
                    )
                lines.append("")
            if review_care.get("exhausted_prs_detail"):
                ...
                for pr in review_care.get("exhausted_prs_detail", []):
                    lines.append(
                        f"| #{pr.get('pr_number', '?')} | {pr.get('branch', '?')} | {pr.get('attempts', 0)} |"
                    )
                lines.append("")
        ...
        lines.extend(
            [
                ...
                "## Metrics",
                "",
                f"- **Total Specks:** {repo.current_findings_count}",
    ...
    def generate_pdf(
        ...
        markdown = self.generate_markdown_report(
            repo, baseline, health, history, findings_by_category, review_care
        )
