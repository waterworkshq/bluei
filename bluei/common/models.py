#!/usr/bin/env python3
"""Cross-layer shared models and utilities.

This module sits between engine/ (lowest layer) and app/ + review/.
Both app/ and review/ import from here to avoid lateral coupling.

Extracted from app/models.py during H5 review→app decoupling (2026-06-18).

Dependency direction:
    common/ → engine/   (uses engine primitives like now_iso)
    app/    → common/   (re-exports for backward compat)
    review/ → common/   (canonical — NEVER reaches into app/)
"""
...
# Re-export now_iso from engine for consumer convenience.
# Canonical home remains bluei/engine/models.py.
from bluei.engine.models import now_iso  # noqa: F401
...
class Repo:
    ...
    Note: ``baseline`` references bluei.app.models.Baseline — kept as
    Any-style forward reference here so common/ does not depend on app/.
    The Baseline dataclass itself is app-specific (used by app/ runner,
    health, dashboard) and stays in app/models.py.
    """
    ...
    baseline: Optional[Any] = None  # app.models.Baseline — kept loose to avoid cycle
