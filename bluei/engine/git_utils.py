"""bluei.engine.git_utils — Git helpers and docs index management."""
...
def _git_last_commit_for_path(repo_path: Path, relative_path: str) -> str:
    rc, out = run_capture(
        ["git", "log", "-n", "1", "--format=%H", "--", relative_path], cwd=repo_path
    )
    if rc != 0:
        return ""
    return out.strip()
...
def refresh_docs_index(
    repo_path: Path, docs_index_file: Path, log_file: Path
) -> List[Dict[str, Any]]:
    ...
    _append_text(
        log_file, f"docs-index: refreshed entries={len(entries)} file={docs_index_file}"
    )
    return entries
...
def get_branch(repo_path: Path) -> str:
    ...
    if config_file.exists():
        ...
        try:
            ...
            _logger.debug("Failed to read repo config.yaml")
    out = subprocess.check_output(
        ["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=str(repo_path), text=True
    )
    return out.strip()
