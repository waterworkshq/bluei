"""bluei.engine.prompts — All LLM prompt render functions."""
...
def render_type_safety_prompt(
    ...
    return '\n'.join(
        [
            ...
            f'- file: `{finding.path}`',
            f'- line: `{finding.line}`',
            '- This code has type safety issues.',
            '',
            '## Objective',
...
def render_complexity_refactor_prompt(
    ...
    return '\n'.join(
        [
            ...
            f'- file: `{finding.path}`',
            f'- line: `{finding.line}`',
            '- This method/function has high cyclomatic complexity and needs to be simplified.',
            '',
            '## Objective',
...
def render_maxlines_refactor_prompt(
    ...
    return '\n'.join(
        [
            ...
            f'- rule: `{finding.rule}`',
            f'- file: `{finding.path}`',
            '- This file exceeds the maximum line count and needs to be split.',
            '',
            '## Objective',
...
def render_claude_fix_prompt(
    ...
    mnemo_directives: Optional[str] = None,
    ...
    # Section 6a: Rule-level cross-finding context
    if rule_history:
        sections.append('## Similar fixes in this codebase')
        for entry in rule_history[:5]:
            date = entry.get('date', 'unknown')
            status = entry.get('changed') or entry.get('broke') or '(no detail)'
            fid_short = entry.get('finding_id', '')[:8]
            sections.append(f'- {date}: {status} [{fid_short}]')
        sections.append('')

    # Section 6b: Rule-specific guidance (from LLM fixable rules config prompt_hint)
    if extra_prompt:
        sections.extend([
    ...
    # Section 6c: Learned fix patterns (from pattern replay mid-confidence match)
    if learned_patterns:
        sections.extend([
    ...
    # Section 6d: Failure pattern clusters
    if failure_clusters:
        sections.extend([
