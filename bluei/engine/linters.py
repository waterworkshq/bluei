"""bluei.engine.linters - Ruff rule cache, xo linter runner, and discovery functions."""
...
from bluei.engine.models import Finding, stable_finding_id
...
def run_xo_linter_in_container(container_name: str = 'ky-phase2-dev') -> Tuple[int, List[Dict[str, Any]]]:
    ...
    try:
        ...
        try:
            ...
            _logger.debug("JSON parse failed, falling back to text parsing")
...
def discover_python_linter_findings(repo_path: Path, log_file: Path) -> List[Finding]:
    ...
    try:
        ...
        _append_text(log_file, 'python-discovery: ruff JSON parse failed, falling back to concise')
        return findings
                ...
                if branches:
                    for branch_id, hits in branches.items():
                        if hits == 0:
                            ...
                            finding = Finding(
                                finding_id=stable_finding_id(str(repo_path), relative_path, line, 'test-coverage-branch', 'Branch not covered'),
                                repo=str(repo_path),
                                path=relative_path,
                                line=line,
