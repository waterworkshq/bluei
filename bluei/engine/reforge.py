"""Refactor-class finding detection, routing, and state scaffolding.

Classifies findings as SIMPLE_FIX (deterministic), REFACTOR_CLASS (multi-file),
or CLAUDE_FIX (needs LLM). RefactorPhase tracks PLANNING→SPLITTING→VALIDATING→DONE
with safety gates limiting autonomous refactor scope.
"""
...
from bluei.engine.constants import MAX_LINES_REFACTOR_LIMIT, MAX_LINES_REFACTOR_TARGET
...
class RefactorClass(str, enum.Enum):
    ...
    #: Multi-engine cascade: tries deterministic engines before LLM.
...
REFACTOR_CLASS_RULES: Set[str] = {
    ...
    "debt-todo-marker",  # TODO/FIXME — intentional debt, not automatable
    "rust-clone-unnecessary",  # 0.60 confidence, use-after-move risk
}
...
def reset_context_rules_cache() -> None:
    global _CONTEXT_RULES_CACHE
    _CONTEXT_RULES_CACHE = None
...
) -> Optional[ContextOverride]:
    """Check if a file path matches any context override for this rule."""
    for ctx in context_rule.contexts:
        if ctx.framework != "any":
            if not detected_frameworks or ctx.framework not in detected_frameworks:
                continue
...
def classify_finding(
    finding, detected_frameworks: Optional[List[str]] = None
) -> RefactorClass:
    """Classify a Finding into one of three fix classes.

    Routing precedence: REFACTOR_CLASS_RULES → CONTEXT_RULES → CLAUDE_FIX_RULES
    → Recipe match → Ruff autofix → CASCADE_FIX.

    See docs/RULES_REFERENCE.md for the full routing table and context rule docs.
    ...
    Args:
        ...
        detected_frameworks: Optional list of detected frameworks for context matching.
    ...
    if context_rule:
        matched = match_context(
            getattr(finding, "path", "") or "", context_rule, detected_frameworks
        )
        if matched:
        ...
        if context_rule.default_strategy in ("deterministic", "deterministic_safe"):
            finding.refactor_class = RefactorClass.SIMPLE_FIX.value
            return RefactorClass.SIMPLE_FIX
    ...
    try:
        ...
        logging.debug("recipe engine lookup failed for rule=%s", rule)
        pass
    if rule.startswith("ruff-") and getattr(finding, "safe_to_autofix", False):
        finding.refactor_class = RefactorClass.SIMPLE_FIX.value
        return RefactorClass.SIMPLE_FIX

    # Default: cascade tries deterministic engines before LLM.
    finding.refactor_class = RefactorClass.CASCADE_FIX.value
    return RefactorClass.CASCADE_FIX
    ...
    if worktree_path and path:
        ...
        if file_path.is_file():
            try:
                ...
                _logger.debug("Failed to check file line count")
    ...
    if rule == "xo-max-lines" and worktree_path and path:
        ...
        if file_path.is_file():
            try:
                ...
                _logger.debug("Failed to estimate refactor target")

    return False


def can_auto_refactor(
    finding, worktree_path: Optional[Path] = None
) -> tuple[bool, str]:
    ...
    if rule == "xo-max-lines" and worktree_path and path:
        ...
        if file_path.is_file():
            try:
                ...
                _logger.debug("Failed to read file for max-lines gate")
...
def describe_class(rc: RefactorClass) -> str:
    ...
    return {
        ...
        RefactorClass.CASCADE_FIX: (
            "Multi-engine cascade — tries deterministic engines (linters, recipes, pattern "
            "replay, AST transforms) in tier order before falling to LLM."
        ),
    }.get(rc, "Unknown")
