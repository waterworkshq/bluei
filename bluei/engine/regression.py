...
def _resolve_current_branch(repo_path: Path) -> str:
    """Return the current git branch name for a given repo path."""
    rc, out = run_capture(
...
def detect_removed_tests(repo_path: Path, main_branch: str) -> list[str]:
    """Detect deleted test files between main_branch and the current branch.

    Args:
        repo_path: Local repository path.
        main_branch: Base branch name (e.g. 'main', 'origin/main').

    Returns:
        List of deleted test file paths relative to repo root.
    """
    if not repo_path.exists():
        return []

    head = _resolve_current_branch(repo_path)
    changes = _git_diff_names(repo_path, main_branch, head)
    ...
    for change in changes:
        ...
        fpath = change['path']
        if any(fpath.startswith(p) for p in test_prefixes):
            deleted_tests.append(fpath)

    return deleted_tests


def detect_export_changes(repo_path: Path, main_branch: str) -> list[str]:
    """Detect removed/renamed public exports between main_branch and current branch.
    ...
    Returns:
        List of human-readable strings describing each export change.
    """
    if not repo_path.exists():
        return []
    ...
    head = _resolve_current_branch(repo_path)
    changes = _git_diff_names(repo_path, main_branch, head)
    findings = _find_export_changes(changes, repo_path)
    ...
    for entry in findings:
        ...
        if etype == 'module_init_deleted':
            result.append(f'Init module deleted: {epath}')
        elif etype == 'export_module_deleted':
            result.append(f'Export module deleted: {epath}')
        else:
            result.append(f'Export change detected: {epath}')

    return result


def compute_regression_score(repo_path: Path, main_branch: str) -> float:
    """Compute a regression score comparing current branch against main.

    Score ranges from 0.0 (clean, no regressions) to 1.0 (major regression).
    Weighted factors:
    - Deleted test files: 0.3 weight (high signal)
    - Deleted export modules: 0.4 weight (highest signal)
    - Deleted inits: 0.2 weight
    - New lint violations: 0.1 weight per violation (capped at 0.3)
    ...
    Returns:
        Float between 0.0 (clean) and 1.0 (severe regression).
    """
    if not repo_path.exists():
        return 0.0
    ...
    # Factor 1: Deleted test files (0.3 max)
    test_dels = _find_test_deletions(changes, repo_path, main_branch)
    test_score = min(len(test_dels) * 0.15, 0.3)

    # Factor 2: Export changes (0.4 max)
    export_changes = _find_export_changes(changes, repo_path)
    export_score = 0.0
    for entry in export_changes:
        if entry['type'] == 'module_init_deleted':
            export_score += 0.2
        elif entry['type'] == 'export_module_deleted':
            export_score += 0.4
    export_score = min(export_score, 0.4)

    # Factor 3: Lint regressions (0.3 max)
    lint_regs = _find_lint_regressions(repo_path, main_branch, head)
    lint_score = min(len(lint_regs) * 0.1, 0.3)

    total = test_score + export_score + lint_score
