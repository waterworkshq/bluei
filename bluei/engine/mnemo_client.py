...
"""Mnemo memory client — fast SQLite-backed context for fix decisions.

Two access paths: Python-native (SQLite, <50ms) via get_context_for_finding(),
and TypeScript CLI (slow, ~10s+) via recall()/seed(). QA agent fix context
should prefer the Python-native path.
"""
...
def _mnemo_config() -> dict:
    ...
    return {
        "max_patterns": cfg.get(
            "max_patterns",
            int(os.environ.get("ENGRAM_MAX_PATTERNS", _MNEMO_DEFAULTS["max_patterns"])),
        ),
        "max_chars": cfg.get(
            "max_chars",
            int(os.environ.get("MNEMO_MAX_CHARS", _MNEMO_DEFAULTS["max_chars"])),
        ),
        "recall_limit": cfg.get(
            "recall_limit",
            int(os.environ.get("MNEMO_RECALL_LIMIT", _MNEMO_DEFAULTS["recall_limit"])),
        ),
        "timeout_seconds": cfg.get(
            "timeout_seconds",
            int(
                os.environ.get(
                    "MNEMO_TIMEOUT_SECONDS", _MNEMO_DEFAULTS["timeout_seconds"]
                )
            ),
        ),
        "max_files": cfg.get(
            "max_files",
            int(os.environ.get("ENGRAM_MAX_FILES", _MNEMO_DEFAULTS["max_files"])),
        ),
        "max_callers": cfg.get(
            "max_callers",
            int(os.environ.get("ENGRAM_MAX_CALLERS", _MNEMO_DEFAULTS["max_callers"])),
        ),
        "max_callees": cfg.get(
            "max_callees",
            int(os.environ.get("ENGRAM_MAX_CALLEES", _MNEMO_DEFAULTS["max_callees"])),
        ),
        "max_deps": cfg.get(
            "max_deps",
            int(os.environ.get("ENGRAM_MAX_DEPS", _MNEMO_DEFAULTS["max_deps"])),
        ),
        "enabled": cfg.get("enabled", os.environ.get("MNEMO_ENABLED", "1") != "0"),
        "cli_path": cfg.get(
            "cli_path", os.environ.get("MNEMO_CLI_PATH", _MNEMO_DEFAULTS["cli_path"])
        ),
    }
...
ENGRAM_MAX_CALLEES = int(os.environ.get("ENGRAM_MAX_CALLEES", "10"))
...
def _project_for(repo_path: Path) -> str:
    ...
    if config_path.exists():
        try:
            ...
            _logger.debug("Failed to read package.json for mnemo")
...
class MnemoClient:
    ...
    def find_relevant_files(
        self, query: str, limit: int = ENGRAM_MAX_FILES
    ) -> List[FileRelevance]:
        ...
        return [
            FileRelevance(
                file_path=row[0],
                match_count=row[1],
                pattern_types=row[2].split(",") if row[2] else [],
            )
            for row in cur.fetchall()
        ]

    def find_relevant_files_for_finding(
        self, finding: Finding, limit: int = ENGRAM_MAX_FILES
    ) -> List[FileRelevance]:
        ...
        for query in queries[:6]:
            for row in self.find_relevant_files(query, limit=max(limit * 3, 10)):
                ...
                if existing is None:
                    ...
                    existing.pattern_types = list(
                        sorted(set(existing.pattern_types + row.pattern_types))
                    )
        ...
        ranked = sorted(
            scored.values(),
            key=lambda fr: (
                fr.match_count + bonus.get(fr.file_path, 0),
                fr.file_path == finding.path,
                -len(fr.pattern_types),
            ),
            reverse=True,
        )
        return ranked[:limit]
    ...
    def get_callers(
        self, symbol: str, limit: int = ENGRAM_MAX_CALLERS
    ) -> List[CallGraphEntry]:
    ...
    def get_callees(
        self, symbol: str, limit: int = ENGRAM_MAX_CALLEES
    ) -> List[CallGraphEntry]:
    ...
    def get_dependencies(
        self, file_path: str, limit: int = ENGRAM_MAX_DEPS
    ) -> List[Dependency]:
    ...
    def get_context_for_finding(self, finding: Finding) -> Optional[str]:
        ...
        same_file = [
            p
            for p in self.search_patterns(finding.rule, limit=5)
    ...
    def seed(
        ...
        outcome_content = f"Fix attempt for {finding.rule} in {finding.path}:{finding.line}\nOutcome: {outcome}"
        if error:
            outcome_content += f"\nError: {error}"
        if changes:
            outcome_content += f"\nChanges: {changes}"
    ...
    def _recall_via_cli(self, finding: Finding) -> Optional[str]:
        ...
        try:
            ...
            _logger.debug("Failed to run mnemo-cli")
