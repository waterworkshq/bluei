"""bluei.engine.state — State, issues, findings persistence and workload reconciliation."""
...
from bluei.engine.gh import fetch_github_live_counts, get_origin_url
...
def append_log(path: Path, msg: str) -> None:
    ...
    Used across the engine as the primary structured logging mechanism.
    Each line is formatted as ``[ISO-8601] message`` for downstream parsing.

    Args:
        path: Log file path (parent directories created automatically).
...
def load_finding_record(
    finding_id: str, findings_file: Path
...
def update_finding_record(
    finding_id: str, findings_file: Path, updates: Dict[str, Any]
) -> bool:
...
def save_refactor_work(
    finding_id: str, findings_file: Path, refactor_work: RefactorWork
) -> bool:
...
def repair_state(path: Path) -> bool:
    """Try to load state.json. If corrupted or missing, write a fresh default state.

    Returns True if repair was needed (state was corrupted or missing),
    False if the state was clean and loadable.
    """
    ...
    if not path.exists():
        save_state(path, _default_state)
        return True
    ...
    try:
        ...
        # State loaded cleanly — no repair needed
        return False

    except (json.JSONDecodeError, Exception):
        logging.warning("state.json corrupted at %s — resetting to defaults", path)
        save_state(path, _default_state)
        return True
