"""Post-merge rebase sweep across sibling PRs.

After a merge lands, rebases every other open PR onto the updated base.
Clean rebases are force-pushed; conflicted ones are flagged and pulled
from merge rotation. Processes oldest PRs first for higher success rate.
"""
...
def _fetch_sibling_prs(
    ...
    rc, out = run_capture(
        [
            ...
            "open",
            "--json",
            "number,headRefName,headRepository,createdAt,body",
            "--limit",
            "50",
        ],
        cwd=cwd,
...
def _rebase_sibling(
    ...
    conflict_files = (
        [f.strip() for f in diff_out.splitlines() if f.strip()] if rc_diff == 0 else []
    )
...
def sweep_rebase(
    ...
    Args:
        ...
        rebase_stats_file: If provided, write structured telemetry to this
            JSONL file via ``log_rebase_stats``.
    ...
    _rebases_attempted = 0
    _rebases_succeeded = 0
    _rebases_conflicted = 0
    _rebases_skipped = 0

    siblings = _fetch_sibling_prs(
    ...
    for pr in siblings:
        ...
        if not head_branch:
            ...
            result["skipped"].append(
                {"pr_number": pr_number, "reason": "no-head-branch"}
            )
            continue
        ...
        if _has_conflict_marker(pr_body):
            ...
            result["skipped"].append(
                {"pr_number": pr_number, "reason": "already-conflict-marked"}
            )
            continue
        ...
        if success:
            ...
            if not detail:
                ...
                result["skipped"].append(
                    {"pr_number": pr_number, "reason": "empty-HEAD-after-rebase"}
                )
                continue
    ...
    _append_text(
        ...
        f"duration={_duration:.2f}s",
    )

    # Persist structured sweep telemetry
    if rebase_stats_file is not None:
        log_rebase_stats(
            rebase_stats_file,
            {
                ...
                "base": base_branch,
                "dry_run": dry_run,
                "rebases_attempted": _rebases_attempted,
                "rebases_succeeded": _rebases_succeeded,
                "rebases_conflicted": _rebases_conflicted,
                "rebases_skipped": _rebases_skipped,
                "duration_seconds": round(_duration, 3),
            },
        )
