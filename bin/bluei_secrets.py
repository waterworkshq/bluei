#!/usr/bin/env python3
"""bluei secrets — Standalone secret scanning using gitleaks.

Usage:
  bluei secrets <repo-path>
  bluei secrets --repo <repo-path> --format json
...
def find_tool(binary: str) -> str:
    ...
    try:
        ...
        _logger.debug("Failed to locate binary via which")
...
def scan_secrets(repo_path: Path, format: str = 'text', verbose: bool = False) -> list:
    ...
    try:
        ...
        try:
            ...
            _logger.debug("Failed to unlink temp report file")
