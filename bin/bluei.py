...
def _cmd_preflight(rest: list[str]):
    from bin.cmd_onboard import _cmd_preflight as _impl
...
def _cmd_report(name: str, passthrough: list[str]):
    ...
    return _impl(name, passthrough)


def _cmd_dashboard(rest: list[str]) -> int:
    from bin.cmd_report import _cmd_dashboard as _impl

    return _impl(rest)


def _cmd_init(rest=None):
    from bin.cmd_onboard import _cmd_init as _impl

    return _impl(rest)


def _cmd_onboard(rest: list[str]):
    from bin.cmd_onboard import _cmd_onboard as _impl

    return _impl(rest)


def _cmd_upgrade_config(rest: list[str]) -> int:
    from bin.cmd_onboard import _cmd_upgrade_config as _impl

    return _impl(rest)


# Re-exports for test monkeypatch compat
from bin.cmd_onboard import (  # noqa: E402, F401
    _confirm_high_risk_mode,
    _parse_init_args,
    _print_safety_summary,
)
