#!/usr/bin/env python3
"""Obsidian log sync for bluei.

Reads current bluei state/run artifacts and writes markdown logs to
...
REPO_HEADERS = {
    "issue-cycle": "## {repo} (bluei/{repo})",
    "pr-cycle": "## {repo} (bluei/{repo})",
    "merge-cycle": "## {repo} (bluei/{repo})",
    "review-cycle": "## {repo} (bluei/{repo})",
    "qa-monitor": "## {repo} (bluei/{repo})",
}
...
def _replace_repo_section(content: str, repo_name: str, new_section: str) -> str:
    ...
    if first_repo_pos > 0:
        ...
        has_new_format = bool(re.match(r"## [\w-]+ \(\S+/\S+\)", first_section_start))
        if has_new_format:
...
def _build_issue_cycle(repo_name: str, date_str: str) -> str:
    ...
    closed_today = [
        i
        for i in issues
        if i.get("status") in ("closed", "completed")
        and i.get("updated_at", "").startswith(date_str)
    ]
    ...
    if stale_findings is not None and abs(stale_findings - findings_count) > 1:
        print(
            f"[WARN] {repo_name}: status.json findings_entries={stale_findings} "
            f"differs from source-of-truth findings.jsonl count={findings_count}",
            file=sys.stderr,
        )
    if stale_open_issues is not None and stale_open_issues != open_issues_count:
        print(
            f"[WARN] {repo_name}: status.json open_issues={stale_open_issues} "
            f"differs from source-of-truth issues.json count={open_issues_count}",
            file=sys.stderr,
        )
    ...
    lines = [
        ...
        f"| Specks Tracked | {findings_count} |",
    ]
    ...
    lines.extend(["", f"*Source: bluei state — {date_str}*"])
    return "\n".join(lines)
...
def _build_pr_cycle(repo_name: str, date_str: str) -> str:
    ...
    if prs:
        ...
        for pr_num, pr_data in sorted(
            prs.items(), key=lambda x: int(x[0]), reverse=True
        )[:20]:
...
def _build_merge_cycle(repo_name: str, date_str: str) -> str:
    ...
    prs = active_prs_data.get("prs", {})
    latest_run_metrics = (
        status_data.get("latest_run_metrics", {})
        if isinstance(status_data, dict)
        else {}
    )
    blocked_reasons = (
        latest_run_metrics.get("blocked_reasons", [])
        if isinstance(latest_run_metrics, dict)
        else []
    )
    ...
    if prs:
        ...
        for pr_num, pr_data in sorted(
            ...
            lines.append(
                f"| {pr_num} | {branch} | {author} | {status} | {merge_state} | {merge_reason} |"
            )
        lines.append("")
...
def _build_review_cycle(repo_name: str, date_str: str) -> str:
    ...
    active = sum(
        1
        for pr in managed_prs.values()
        if pr.get("status") in ("awaiting_review", "in_progress")
    )
    pending_push = sum(
        1 for pr in managed_prs.values() if pr.get("status") == "pending_push_approval"
    )
    ready = sum(1 for pr in managed_prs.values() if pr.get("status") == "merge_ready")
    failed = sum(
        1
        for pr in managed_prs.values()
        if pr.get("status") in ("failed_push", "retry_failed")
    )
        ...
        for run in review_runs[:5]:
            ...
            lines.extend(
                [
                    ...
                    f"Specks: {run.get('findings_detected', 0)} | "
    ...
    if prs:
        ...
        for pr_num, pr_data in sorted(
            ...
            lines.append(
                f"| {pr_num} | {branch} | {status} | {merge_state} | {review_notes} |"
            )
        lines.append("")
    else:
...
def sync_phase(
    repo_name: str, phase: str, date_str: Optional[str] = None, dry_run: bool = False
) -> bool:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sync bluei state to Obsidian logs")
    ...
    parser.add_argument(
        "--phase", choices=list(CYCLE_SUBDIRS.keys()), help="Specific phase to sync"
    )
