#!/usr/bin/env python3
"""bluei watchdog — inspects latest run state and logs for all registered repos."""
import json
import os
import subprocess
import sys
...
def latest_log_block(lines):
    """Return only the latest run block from a bluei repo log.
...
def check():
    ...
    if not REPOS:
        return "No repos configured. Run 'bluei init' or 'bluei onboard --repo <path>' to add one."
    ...
    for repo in REPOS:
        ...
        if not rd.exists():
            ...
            alerts.append(f"  Repo is registered but never initialized. Run 'bluei scan {repo}' to create state.")
            continue
        ...
        log_path = ROOT / "logs" / f"bluei-{repo}.log"
        if log_path.exists():
        ...
        try:
            ...
            if lines:
                ...
                if latest.get("findings_failed",0) > 0:
                    alerts.append(f'  ⚠ specks failed: {latest.get("findings_failed",0)}')
