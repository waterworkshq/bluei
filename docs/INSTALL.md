# Blenny — Standalone Install

## Goal

Set up Blenny (codebase `qa-agent`) on a machine without relying on OpenClaw runtime features.
...
```bash
npm install -g blenny   # installs the JS wrapper + Python bootstrap
blenny init             # interactive first-run setup
```
