...
├── constants.py                # Add CONTEXT_RULES catalog (see RULES_REFERENCE.md for all rules)
└── tests/
...
./bin/bluei run --repo zulip --phase pr-cycle --no-dry-run

# Optional: one-shot context re-evaluation for existing issues (Phase 4)
./bin/bluei run --repo zulip --phase context-reconcile --no-dry-run
```
