...
---
*Generated by bluei batch PR engine*
```
...
| File | Change |
|------|--------|
| `bluei/engine/batch_pr.py` | **NEW** — Batch grouping engine, execution, PR creation |
| `bluei/engine/batch_rules.yaml` | **NEW** — Declarative grouping rules |
| `bluei/engine/models.py` | Add `BatchGroup` dataclass, `BatchStatus` enum |
| `bluei/engine/state.py` | Add batch state persistence (load/save batches) |
| `bluei/engine/cli.py` | Wire batch engine into pr-cycle; fallback to solo |
| `bluei/engine/constants.py` | Add `BATCH_RULES` catalog, defaults |
| `bluei/engine/gh.py` | Add `create_batch_pr()` helper |
