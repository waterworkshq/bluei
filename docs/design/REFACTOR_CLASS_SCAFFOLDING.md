<!-- CATEGORY: design -->
<!-- STATUS: historical reference — describes design intent, not current operational state -->
...
This module provides the **first scaffolding slice** for autonomous large-refactor handling in the bluei workflow. It establishes:
...
```
python3 bluei/engine.py --run-phase refactor-cycle --no-dry-run
```
...
| File | Change |
|------|--------|
| `bluei/engine/reforge.py` | **NEW** — RefactorClass, RefactorPhase, RefactorWork, classify_finding, can_auto_refactor, is_large_refactor, describe_class |
| `bluei/engine/refactor_queue.py` | **NEW** — durable queue entries, statuses, enqueue/approve/execute/fail helpers |
| `bluei/engine/models.py` | Added `refactor_class` and `refactor_phase` fields to `Finding`; updated `as_dict`/`from_dict` |
| `bluei/engine/state.py` | Added `load_refactor_work()`, `save_refactor_work()`, `get_pending_refactor_work()` |
| `bluei/engine/orchestrator.py` | Added `route_findings_with_intent()` and `build_refactor_cycle_command()` |
| `bluei/engine/lifecycle.py` | Added queue routing / processing helpers including `route_to_human_review()` and `process_refactor_queue()` |
| `bluei/engine/cli.py` | Added live refactor-cycle run path and queue-processing flags |
| `bluei/engine/__init__.py` | Re-exported routing, persistence, and queue public API |
| `bluei/engine/test_reforge.py` | Classification, state machine, safety gate coverage |
| `bluei/engine/test_refactor_queue.py` | Queue behavior coverage |
| `bluei/engine/test_route_findings_with_intent.py` | Intentional routing coverage |
| `bluei/engine/test_refactor_state.py` | RefactorWork persistence coverage |
...
| `bluei/engine/enforce_architecture.py` | Updated expected modules / legal imports |
| `bluei/engine/check_completeness.py` | Updated intentional package-only symbol allowlist |
