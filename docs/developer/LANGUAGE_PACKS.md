...
### Graduated Packs (`plugins/graduated-*/`)

Self-contained `DiscoveryPlugin` packs generated by `bluei/tools/graduator/`
(alpha.5) from graduated ACTIVE emergent rules. Unlike the 7 language packs
above (which extend `ToolWrapperPlugin` to wrap an external linter CLI),
graduated packs extend `DiscoveryPlugin` directly — their `discover()` embeds
the rule's `DetectionPattern` (TEXT/REGEX/STRUCTURAL dispatch) and emits
`Finding` instances without any external tool subprocess. No `health_mapping.yaml`
(no linter rules to map). Generated by `generate_plugin_pack()` (build-time,
ACTIVE-only gate); loaded by `PluginLoader` unchanged. The first committed
worked example is `plugins/graduated-eslint/` (eslint-no-console, REGEX).
