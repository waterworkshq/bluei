...
| `emergent_rules.py` | Proposes new detection rules from repeated finding patterns. Detection types: TEXT_PATTERN, REGEX_PATTERN (ADR-0021), STRUCTURAL (AST-based, Python-only, alpha.5). `extract_detection_ast` populates the structural fingerprint; `scan_shadow_rules` + `measure_false_positives` dispatch on type. |
| `campaign_evidence.py` | App-layer consumer for campaign emergent-rule evidence (injected into `CampaignExecutor` as a callable to keep campaigns free of app imports). Advances target rules + proposes new rules from successful fix patterns. Zero governance ApprovalRecord writes. |
...
| `prompts.py` | LLM prompt rendering with directive seeding context injection (10 context channels incl. authoritative guidelines §6e + repo taste §6f). |
...
| `repo_taste_loader.py` | Loads repo taste entries (framework conventions) from `repo_taste/` into LLM fix prompts by framework + language (§6f). Mirrors `guideline_loader`; prompt-context, NOT governed (ADR-0017). |
...
| `model_governor.py` | Model Governor (ADR-0022): tier-selection function that recommends a Routing Ladder tier (tier-0/1/2/escalate) per Finding based on rule-family deterministic coverage. As of beta.1 the cycle default is `select_tier` (act-on-recommendation — real recommendations); the actual model downgrade is gated on operator `model_tiers.yaml` presence, so under no operator config the invocation is byte-identical to alpha.6. `resolve_governed_model` is the shared helper both LLM call sites (pr-cycle + batch) use to record + resolve. Emits only tier ordinals — never model ids. |
| `model_discovery.py` | Model Discovery (ADR-0022 amendment 2; CONTEXT "Model Discovery"): resolves a Routing Ladder tier to a concrete model id at invocation. `BackendModelDiscovery` validates an operator-supplied tier→model mapping (`model_tiers.yaml`) against the active backend (`claude`/`opencode`) CLI's real catalog — no shipped defaults (vendor ids rot). `inject_model_flag` splices `--model <id>` into the command template (backend auto-detected). `MockModelDiscovery` (in `tools/benchmark/`) is the benchmark/test fixture. |
...
**Self-contained graduated packs** (`plugins/graduated-*/`, alpha.5): build-time
generated by `bluei/tools/graduator/` from graduated ACTIVE emergent rules. The
first real `DiscoveryPlugin` subclass that does NOT wrap an external tool — its
`discover()` embeds the rule's `DetectionPattern` (TEXT/REGEX/STRUCTURAL dispatch)
directly and emits `Finding` instances. Generated by `generate_plugin_pack()`;
loadable by `PluginLoader` unchanged. See `docs/developer/LANGUAGE_PACKS.md` for
the plugin contract.
...
- `types.py` — `LearningObjective` (alpha.5): a declared evidence target (emergent-rule / pattern-family / recipe) that routes per-finding outcomes to the matching native evidence layer. When set, the executor captures evidence (emergent-rule → injected `campaign_evidence.py` consumer; pattern-family → Dry Replay) and produces a `learning_report`. Zero governance ApprovalRecord writes — campaigns are evidence sources, not governance decision-makers.
...
5. **Promote** — retire emergent rule and write as a self-contained graduated
   plugin pack via `bluei/tools/graduator/` (alpha.5)

Detection types (alpha.5): TEXT_PATTERN (substring), REGEX_PATTERN (ADR-0021),
STRUCTURAL (AST shape-matching, Python-only — `extract_detection_ast` produces
a JSON fingerprint; `scan_shadow_rules` matches via exact hash + fuzzy fallback).
REGEX/TEXT detection paths are additive — all three coexist.
...
~6,463 tests across 220+ files (2026-06-30). All use `tmp_path` fixtures — no external services
needed. Run with:
