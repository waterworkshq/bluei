...
- **AST STRUCTURAL was deferred to alpha.5; FULFILLED in v0.2.0-alpha.5 (2026-06-30).** alpha.5 activates `DetectionType.STRUCTURAL` for Python via `extract_detection_ast` (JSON structural fingerprint: hash/nodes/operators/canonical_snippet) + a `scan_shadow_rules` STRUCTURAL branch (exact structural hash + fuzzy fallback, mirroring the Pattern-replay lookup) + a `measure_false_positives` STRUCTURAL branch. Both proposers now prefer AST-first for Python (STRUCTURAL) and fall back to regex/text. **Python-only** scope refinement (Phase 1 C2) — TS/JS STRUCTURAL + a TS structural hasher deferred to future-cross-cutting (regex detection covers TS meanwhile). Regex detection stays additive — TEXT/REGEX branches unchanged. Note: `compute_structural_hash` is node-shape-invariant (normalizes identifier names + literal values, distinguishes Name vs Constant), not fully rename-invariant — same-shape renames match; different-arg-shape (e.g. Name+Constant vs Name+Name) does NOT exact-hash-match (fuzzy fallback may still catch near-shapes).
